"""
Training script for NP-SNN orbital mechanics model.

Complete training pipeline with curriculum learning, physics-informed losses,
and MLflow experiment tracking. Follows implementation plan for production-grade
space debris detection system.

Usage:
    python scripts/train_npsnn.py --config configs/space_debris_simulation.yaml
    python scripts/train_npsnn.py --config configs/space_debris_simulation.yaml --resume checkpoints/checkpoint_epoch_50.pt
"""
...
from src.data.orbital_dataset import OrbitalDataset
...
def create_datasets(config: TrainingConfig, device: torch.device) -> tuple:
    ...
    print("📊 Generating orbital datasets...")
    ...
    train_trajectories = []
    for i in range(0, config.dataset_config.n_trajectories_train, 100):  # Generate in batches
        batch_size = min(100, config.dataset_config.n_trajectories_train - i)
        batch_trajectories = data_generator.generate_batch(
            batch_size=batch_size,
            add_observation_gaps=True,
            gap_probability=0.3
        )
        train_trajectories.extend(batch_trajectories)
        
        if (i + batch_size) % 500 == 0:
            print(f"      Generated {i + batch_size:,}/{config.dataset_config.n_trajectories_train:,} training trajectories")
    
    # Generate validation trajectories
    print(f"   Generating {config.dataset_config.n_trajectories_val:,} validation trajectories...")
    val_trajectories = []
    for i in range(0, config.dataset_config.n_trajectories_val, 100):
        batch_size = min(100, config.dataset_config.n_trajectories_val - i)
        batch_trajectories = data_generator.generate_batch(
            batch_size=batch_size,
            add_observation_gaps=True,
            gap_probability=0.2  # Less challenging gaps for validation
        )
        val_trajectories.extend(batch_trajectories)
...
def main():
    parser = argparse.ArgumentParser(description='Train NP-SNN for orbital mechanics')
    ...
    print("🚀 NP-SNN Orbital Mechanics Training Pipeline")
    print("=" * 60)
